...
const buildHtml = ({ topRecs, alerts, frontendUrl }) => {
  const rec = topRecs
    .map(
      (r) => `
  ...
  const alertList = alerts
    .map(
      (a) => `
        <li style="margin-bottom:8px;color:${a.severity === "high" ? "#c0392b" : "#b8860b"};">
          <strong>${escape(a.title)}</strong>
          <span style="font-size:11px;text-transform:uppercase;margin-left:6px;">${escape(a.severity)}</span>
          <p style="margin:2px 0 0 0;color:#444;font-size:13px;">${escape(a.message)}</p>
        </li>`
    )
    .join("");

  const recsLink = frontendUrl ? `${frontendUrl}/admin/recommendations` : "your admin panel";
  const alertsLink = frontendUrl ? `${frontendUrl}/admin/alerts` : "your admin panel";
  ...
  return `
    <div style="font-family:Arial,sans-serif;max-width:680px;margin:0 auto;color:#222;">
      ...
      <p style="color:#555;margin-top:0;">Top recommendations and active alerts for the week.</p>

      ${alerts.length > 0 ? `
        <h3 style="color:#0a0a0a;margin-top:24px;">Active alerts (${alerts.length})</h3>
        <ul style="padding-left:20px;">${alertList}</ul>
        <p style="margin-top:8px;"><a href="${alertsLink}" style="color:#b8941d;">Review all alerts →</a></p>
      ` : ""}

      ${topRecs.length > 0 ? `
        <h3 style="color:#0a0a0a;margin-top:24px;">Top recommendations</h3>
        <ul style="padding-left:20px;">${rec}</ul>
        <p style="margin-top:8px;"><a href="${recsLink}" style="color:#b8941d;">See full recommendations dashboard →</a></p>
      ` : ""}

      ${alerts.length === 0 && topRecs.length === 0 ? `
...
const sendDigestNow = async () => {
  const [topRecs, alerts, admins] = await Promise.all([
    collectTopRecommendations(),
    SmartAlert.find({ acknowledged: false }).sort({ severity: 1, createdAt: -1 }).limit(10).lean(),
    User.find({ role: { $in: ["admin", "super_admin"] }, email: { $exists: true, $ne: "" } })
      .select("email name")
      .lean(),
  ]);

  if (topRecs.length === 0 && alerts.length === 0) {
  ...
  const subject = `[Saga Elite] Weekly AI Digest — ${topRecs.length} recommendation${topRecs.length === 1 ? "" : "s"}, ${alerts.length} active alert${alerts.length === 1 ? "" : "s"}`;
