...
const PaymentReference = ({ referenceNumber, expiresAt, onCopy }) => {
  ...
  return (
    <div className="rounded-[24px] border border-ink/10 bg-page p-5 shadow-[0_24px_80px_rgba(0,0,0,0.35)]">
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="se-label text-[10px] tracking-[0.28em] text-gray-500">Payment Reference</p>
          <div className="mt-3 flex items-center gap-3">
            <div className="rounded-2xl border border-gold-ink2/20 bg-gold-deep/10 px-4 py-3 font-mono text-lg tracking-[0.22em] text-gold-ink2">
            ...
            {referenceNumber ? (
              <button
                type="button"
                onClick={handleCopy}
                className="inline-flex h-[44px] items-center gap-2 rounded-[16px] border border-ink/10 bg-ink/[0.03] px-4 text-sm font-semibold text-muted transition-all duration-200 hover:-translate-y-0.5 hover:border-gold-ink2/40 hover:text-gold-ink2"
              >
        ...
        <div className="rounded-2xl border border-ink/10 bg-black/40 px-3 py-2 text-right">
