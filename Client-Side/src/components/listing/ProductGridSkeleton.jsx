...
const Tile = ({ tall = false, shimmer = true }) => (
  <div className="bg-page flex flex-col">
    <div
      className={`relative w-full overflow-hidden bg-panel rounded-[1rem] border border-card ${
        tall ? "aspect-square" : "aspect-[3/4]"
      }`}
    >
      {shimmer && (
        <motion.div
          className="absolute inset-0 bg-gradient-to-r from-transparent via-ink/[0.04] to-transparent"
          initial={{ x: "-100%" }}
          animate={{ x: "100%" }}
    ...
    <div className="mt-3 px-2 space-y-2 pb-2">
      <div className="h-3 bg-card w-3/5 rounded-sm" />
      <div className="h-3 bg-card w-1/3 rounded-sm" />
    </div>
  </div>
);
...
const ProductGridSkeleton = ({ count = 12, featuredEvery = 7 }) => {
  ...
  return (
    <div className="grid gap-px bg-line/40 border border-line/40 grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 [grid-auto-flow:dense] [grid-auto-rows:1fr]">
      {tiles.map((_, idx) => {
        const isFeatured =
