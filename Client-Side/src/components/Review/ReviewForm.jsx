...
const ReviewForm = ({ productId, orderId, onSubmit, onCancel, initialValues }) => {
  ...
  return (
    <form
      ...
      className="space-y-6 rounded-3xl border border-ink/10 bg-page p-6"
    >
      <div>
      ...
      <div className="space-y-2">
        <label className="text-xs uppercase tracking-[0.2em] text-ink/60">Title</label>
        <input
          value={title}
          onChange={(event) => setTitle(event.target.value.slice(0, 100))}
          className="w-full rounded-2xl border border-ink/10 bg-black/30 px-4 py-3 text-sm text-ink placeholder:text-ink/40 focus:border-gold-ink2/50 focus:outline-none"
          placeholder="Summarize your experience"
          required
        />
        <div className="text-right text-xs text-ink/40">{titleCount}/100</div>
      </div>

      <div className="space-y-2">
        <label className="text-xs uppercase tracking-[0.2em] text-ink/60">Review</label>
        <textarea
          value={content}
          onChange={(event) => setContent(event.target.value.slice(0, 500))}
          className="min-h-[140px] w-full rounded-2xl border border-ink/10 bg-black/30 px-4 py-3 text-sm text-ink placeholder:text-ink/40 focus:border-gold-ink2/50 focus:outline-none"
          placeholder="Tell us what you loved or what could be improved"
          required
        />
        <div className="text-right text-xs text-ink/40">{contentCount}/500</div>
      </div>

      <div>
        <div
          className="flex flex-col items-center justify-center gap-3 rounded-2xl border border-dashed border-ink/20 bg-black/20 px-6 py-10 text-center"
          onDrop={handleDrop}
        ...
        >
          ...
          <button
            ...
            className="rounded-full border border-ink/20 px-4 py-2 text-xs font-semibold uppercase tracking-[0.2em] text-ink/70"
          >
            Browse files
          </button>
          <input
        ...
        {images.length > 0 && (
          <div className="mt-4 grid gap-3 sm:grid-cols-3">
            {images.map((img) => (
              <div key={img.id} className="rounded-2xl border border-ink/10 bg-black/40 p-3">
                <div className="relative h-24 overflow-hidden rounded-xl">
                  <img src={img.preview} alt="Review" className="h-full w-full object-cover" />
                  <button
                    type="button"
                    onClick={() => removeImage(img.id)}
                    className="absolute right-2 top-2 rounded-full bg-black/60 p-1 text-ink"
                  >
                    <X className="h-3 w-3" />
                  </button>
                </div>
      ...
      <div className="flex flex-wrap gap-3">
        <button
          ...
          className="inline-flex items-center gap-2 rounded-full bg-gold-deep px-6 py-3 text-xs font-semibold uppercase tracking-[0.3em] text-black disabled:cursor-not-allowed disabled:opacity-60"
        >
        ...
        {onCancel && (
          <button
            type="button"
            onClick={onCancel}
            className="rounded-full border border-ink/20 px-6 py-3 text-xs font-semibold uppercase tracking-[0.3em] text-ink/70"
          >
            Cancel
          </button>
        )}
