    ...
    <header className="sticky top-0 z-50 w-full bg-black text-white border-b border-[#D4AF37]/20 shadow-sm">
      <div className="container mx-auto px-4 md:px-6 h-20 flex items-center justify-between">
        ...
        <Link to="/shopping/home" className="flex items-center gap-3">
          <img src="/Logo.jpeg" alt="Saga Elite Logo" className="h-12 w-12 object-cover rounded-md" />
      ...
      {menuOpen && (
        <div className="md:hidden bg-[#0a0a0a] border-t border-[#D4AF37]/10 px-6 py-4 flex flex-col gap-4 text-sm font-medium uppercase tracking-widest">
          ...
          <Link to="/shopping/product-list?category=drops" onClick={() => setMenuOpen(false)} className="hover:text-[#D4AF37] transition-colors">Latest Drop</Link>
          <Link to="/shopping/product-list?category=unisex" onClick={() => setMenuOpen(false)} className="hover:text-[#D4AF37] transition-colors">Unisex</Link>
          <Link to="/shopping/product-list?category=archive" onClick={() => setMenuOpen(false)} className="hover:text-[#D4AF37] transition-colors text-gray-500">Archive</Link>
        </div>
      )}
    </header>
  );
