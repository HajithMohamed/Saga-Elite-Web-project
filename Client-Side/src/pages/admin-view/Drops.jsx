...
import { Package, Calendar, Tag, CheckCircle, Archive, Plus } from 'lucide-react'
        ...
        <Fragment>
            <div className="mb-8 flex w-full justify-between items-center px-4 sm:px-0">
                <div className="flex flex-col gap-1">
                    <h1 className="text-3xl font-serif font-bold text-[#D4AF37]">Drop Management</h1>
                    <p className="text-gray-400 text-sm tracking-wide uppercase">Manage your limited release drops</p>
                </div>
                <Button 
            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                {dropList && dropList.length > 0 ? (
                    dropList.map((drop) => (
                        <div key={drop.id} className="group relative overflow-hidden bg-black/40 border border-gray-800 rounded-xl p-6 hover:border-[#D4AF37]/50 transition-all duration-300">
                            ...
                            {drop.images && drop.images.length > 0 && (
                                <div className="absolute top-0 left-0 w-full h-32 opacity-20 group-hover:opacity-30 transition-opacity">
                                    <img src={drop.images[0].url} alt={drop.name} className="w-full h-full object-cover" />
                            ...
                            <div className="relative z-10 flex flex-col h-full">
                                <div className="flex justify-between items-start mb-4">
                                    ...
                                    <div className="flex gap-2">
                                        <span className={`px-2 py-1 rounded-full text-[10px] uppercase font-bold tracking-wider ${drop.isPublished ? "bg-green-500/10 text-green-500" : "bg-yellow-500/10 text-yellow-500"}`}>
                                ...
                                <div className="space-y-2 mt-auto text-xs text-gray-500 font-medium uppercase tracking-wider border-t border-gray-800 pt-4">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span>Starts: {new Date(drop.releaseDate).toLocaleDateString()}</span>
                                    </div>
                                    {drop.endDate && (
                                        <div className="flex items-center gap-2">
                                            <Archive className="h-3 w-3" />
                                            <span>Ends: {new Date(drop.endDate).toLocaleDateString()}</span>
                                        </div>
                                    )}
                                </div>
                            </div>
                   ...
                   <div className="col-span-full flex flex-col items-center justify-center p-10 border border-dashed border-gray-800 rounded-xl bg-black/20">
                        ...
                        <p className="text-sm text-gray-600 mb-6">Create your first drop to get started.</p>
                        <Button onClick={() => setOpenCreateDialog(true)} variant="outline" className="border-[#D4AF37] text-[#D4AF37] hover:bg-[#D4AF37] hover:text-black">
                            Create Drop
                        </Button>
                    </div>
                )}
