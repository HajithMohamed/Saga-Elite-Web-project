...
const Register = () => {
  ...
  return (
    <div className="relative overflow-hidden w-full max-w-[900px] mx-auto flex flex-col xl:flex-row gap-12 items-center">
      
      {/* LEFT SIDE - BENEFITS */}
      <motion.div 
        className="hidden xl:flex flex-col flex-1 pl-8 border-l border-[#333333]"
        initial={{ opacity: 0, x: -20 }}
        animate={{ opacity: 1, x: 0 }}
        transition={{ duration: 0.5, delay: 0.1 }}
      >
        <Eyebrow tone="gold" size="sm" className="mb-6">Why Join Us?</Eyebrow>
        <ul className="space-y-4">
          {BENEFITS.map((benefit, i) => (
            <motion.li 
              ...
              className="flex items-center gap-3 text-[#d0c5af] se-body text-sm"
            >
              <span className="flex-shrink-0 flex items-center justify-center w-5 h-5 rounded-full bg-[#f2ca50]/10 text-[#f2ca50]">
        ...
        <div className="mt-12 p-6 bg-[#111] border border-[#222]">
          <p className="se-body text-xs leading-relaxed text-[#99907c] italic">
            "Saga Elite creates limited-edition fashion for bold identities. Gain access to the world that never settles."
          </p>
        </div>
      </motion.div>
      ...
      >
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <p className="mt-5 se-body text-sm md:text-base text-[#d0c5af] leading-relaxed xl:hidden">
        ...
        <form onSubmit={handleSubmit} noValidate className="mt-10 space-y-6">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
            ...
            />
            {formData.password && <div className="mt-2"><PasswordStrengthMeter password={formData.password} /></div>}
          </motion.div>
        ...
        {(GOOGLE_ENABLED || FACEBOOK_ENABLED) && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}>
            <div className="my-8 flex items-center gap-4 xl:w-full">
              <Hairline tone="soft" />
              <span className="se-label text-[10px] tracking-[0.28em] text-[#574500]">Or continue with</span>
              <Hairline tone="soft" />
            </div>
        ...
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.6 }} className="mt-6 flex items-center justify-center gap-2 text-xs text-[#99907c]">
          <ShieldCheck size={14} className="text-[#f2ca50]" />
          <span>Secure & Encrypted Authentication</span>
        </motion.div>

        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.7 }} className="mt-10 se-body text-sm text-[#99907c] text-center">
